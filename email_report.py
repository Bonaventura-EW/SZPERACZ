...
Sends summary table + Excel attachment every Monday.
"""
...
def build_report_html():
    ...
    html += f"""<div class="footer">
        <p>Wygenerowano automatycznie przez SZPERACZ OLX • {today.strftime('%d.%m.%Y %H:%M')}</p>
    ...
    if os.path.exists(EXCEL_PATH):
        try:
            with open(EXCEL_PATH, "rb") as f:
                ...
                part.add_header("Content-Disposition", f'attachment; filename="szperacz_olx_{today.strftime("%Y%m%d")}.xlsx"')
                msg.attach(part)
